...
## 01. diff
  **描述**：compare files line by line, 以逐行的方式比较给定的两个文本文件的不同处
...
## 02.	vimdiff
  ...
  **描述**：:edit two or more  file with Vim and show differences, 用vim编辑器同时打开两到四个不同版本的文件，并且在不同之处高亮显示  
...
## 03.	more
  ...
  **描述**： file perusal filter for crt viewing . 用来查看一个文本文件的内容，当填满一屏后停下，可以用快捷键来控制显示内容  
...
## 05.	head
  ...
  **描述**： output the first part of files, 打印出文件的前10（默认）行内容
...
##  06.	tail
  ...
  **描述**： Output the last part of files, 打印文件最后10行的内容
...
**描述**： Concatenate files and print on the standard output, 连接文件并打印到标准输出设备上，用来显示文件的内容，内容会全部显示。  
...
## 08.	xargs
  ...
  **描述**： build and execute command lines from standard input, 将标准输入数据转换成命令行参数。 也可能将单行或者多行文件输入转换成其它格式。
...
## 09.	seq	
**描述**：Print a sequence of numbers 打印从first到last步找为increment连续的整数
